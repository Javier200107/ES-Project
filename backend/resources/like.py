...
class Like(Resource):
    ...
    def get(self, account, post):
        post = PostsModel.get_by_id(post)
        ...
        if acc and post:
            ...
            if accounts:
                for i in accounts:
                    if i.id == acc.id:
                        return {'message': "Post with Id [{}] have a like from user with Id [{}] ".format(post, acc.id)}, 200 if i else 404
                return {'message': "Post with Id [{}] doesn't have likes from user with Id [{}] ".format(post, acc.id)}, 404
            else:
        ...
        with lock.lock:
            ...
            if acc and pt:
                ...
                if accounts:
                    for i in accounts:
                        if i.id == acc.id:
                            return {'message': "Post with Id [{}] can't have two likes from account with id [{}]".format(post, acc.id)}, 404
                accounts.append(acc)
                try:
                    pt.save_to_db()
        ...
        with lock.lock:
            ...
            if acc and pt:
                ...
                if accounts:
                    ...
                    return {'message': "Like with accountId [{}] and postId [{}] doesn't exists".format(acc.id, post)}, 404
                else:
                    return {'message': "Post with Id [{}] doesn't have likes".format(post)}, 404
            else:
                return {'message': "Post with Id [{}] or Account doesn't exist".format(post)}, 404
