...
class Config(object):
    ...
    RECREATE_DB_ON_STARTUP = config_decouple(
        "RECREATE_DB_ON_STARTUP", cast=bool, default=False
    )
    SQLALCHEMY_TRACK_MODIFICATIONS = False
    SECRET_KEY = config_decouple(
...
class ProductionConfig(Config):
    DEBUG = False
    SQLALCHEMY_DATABASE_URI = config_decouple(
        "SQLALCHEMY_DATABASE_URI", default=_AzureDB
    )
...
class DevelopmentConfig(Config):
    DEBUG = True
    SQLALCHEMY_DATABASE_URI = config_decouple(
        "SQLALCHEMY_DATABASE_URI", default="sqlite:///data.db"
    )
...
environment = (
    configurations["development"] if not _PRODUCTION else configurations["production"]
)
